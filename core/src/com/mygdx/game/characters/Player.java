...
public abstract class Player {
    ...
    public Player(World world, int playerNumber, MainScreen screen, ControlScheme cs, int x, int y) {
        ...
        unbreakableAnimations = new HashSet<>();
        unbreakableAnimations.add(normalAttackAnimation);
        unbreakableAnimations.add(eAttackAnimation);
        unbreakableAnimations.add(hitAnimation);
        unbreakableAnimations.add(deathAnimation);
    ...
    protected float getAttackScale(ATTACK_TYPE attack) {
        switch (attack) {
            case NORMAL_ATTACK: return NORMAL_ATTACK_SCALE;
            case E_ATTACK: return E_ATTACK_SCALE;
            case Q_ATTACK: return Q_ATTACK_SCALE;
            default: return 0f;
        }
    }
